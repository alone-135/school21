...
#define ROCKET1_START_Y (HEIGHT_SCRING / 2)
#define ROCKET2_START_Y (HEIGHT_SCRING / 2)
...
void moveBall(int *ball1_x, int *ball1_y, int *vector_X, int *vector_Y, const int *rocket_1_Y, const int *rocket_2_Y);
void screenRendering(const int *rocket_1_Y, const int *rocket_2_Y, const int *ball1_x, const int *ball1_y, int *scPlayer1, int *scPlayer2);
    ...
    for (int y = 0; y < HEIGHT_SCOPE; ++y) {
        for (int x = 0; x < WIDTH_SCRING; ++x) {
            if ((y == 0 && x != 0 && x != WIDTH_SCRING - 1) || (y == HEIGHT_SCOPE - 1 && x != 0 && x != WIDTH_SCRING - 1)) {
                printf("-");
            } else if (((x == 0 && y != 0 && y != HEIGHT_SCOPE - 1) || (x == WIDTH_SCRING - 1 && y != 0 && y != HEIGHT_SCOPE - 1))) {
                printf("|");
...
void moveBall(int *ball1_x, int *ball1_y, int *vector_X, int *vector_Y, const int *rocket_1_Y, const int *rocket_2_Y) {
    ...
    if (*ball1_x == ROCKET1_DEFAULT_X + 1 && (*ball1_y == *rocket_1_Y - 1 ||*ball1_y == *rocket_1_Y  || *ball1_y == *rocket_1_Y + 1)) {
        *vector_X = -(*vector_X);
    }
    if (*ball1_x == ROCKET2_DEFAULT_X - 1 && (*ball1_y == *rocket_2_Y - 1 || *ball1_y == *rocket_2_Y  || *ball1_y == *rocket_2_Y + 1)) {
        *vector_X = -(*vector_X);
    }
    *ball1_y += *vector_Y;
    *ball1_x += *vector_X;
