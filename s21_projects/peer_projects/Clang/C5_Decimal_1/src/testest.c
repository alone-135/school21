...
START_TEST(s21_is_greater_2) {
    ...
    setSign(&value_1, 1);
    int return_value = s21_is_greater(value_1, value_2);
    int res = getAbsoluteGreaterOrEqual(value_1, value_2);
...
START_TEST(s21_mul_7) {
    s21_decimal value_1 = {{1000, 2300, 1, 0}};
    setScale(&value_1, 20);
    s21_decimal value_2 = {{10000, 10, 1, 0}};
    setScale(&value_2, 10);
    s21_decimal result;
    s21_decimal check = {{0, 0, 0, 0}};
    int return_value = s21_mul(value_1, value_2, &result);
...
START_TEST(s21_from_int_to_decimal_1) {
    ...
    ck_assert_int_eq(s21_is_equal(result, check), 1);
    ck_assert_int_eq(return_value, CORRECT);
}
END_TEST
